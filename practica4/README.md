# Exercicis pràctica 4

* [Regles IP default](./ip-default.sh)

* [Exercici 1: Input](./ip-01-input.sh)

* [Exercici 2: Output](./ip-02-output.sh)

* [Exercici 3: Established](./ip-03-established.sh)

* [Exercici 4: ICMP](./ip-04-icmp.sh)

* [Exercici 5: NAT](./ip-05-nat.sh)

* [Exercici 6: Forward](./ip-06-forward.sh)

* [Exercici 7: Port forward](./ip-07-port-forward.sh)

* [Exercici 8: DMZ](./ip-08-dmz.sh)

* [Exercici 9: DMZ2](./ip-09-dmz2.sh)

* [Exercici 10: Drop](./ip-10-drop.sh)
